...
3. post-seedfix best checkpoint 长复评：对完整重跑保存下来的 `best.pt` 做 100/500 局复评，是目前最推荐用于答辩的 checkpoint 对比结果。

### 13.1 post-seedfix final network 结果
...
- 当前 trainable 配置每次最终评估只有 10 局，方差较大；所以更稳妥的口径应结合下一节 `best.pt` 500 局复评。

面试时推荐结论：

> 修复随机动作 seeding 后，我重新跑了四组完整实验。最终网络口径下，随机基线 50 局捕获率是 34%；两个 DQN 最终网络在 10 局评估里都是 10%，MAPPO 最终 10 局达到 70%。但 10 局评估方差较大，所以我又补了 best checkpoint 的 500 局复评，作为更稳妥的答辩结果。

### 13.2 post-seedfix best checkpoint 长复评

新增批量复评命令会扫描一次完整训练输出目录下所有包含 `best.pt` 的子目录，并分别写出每个 checkpoint 的 `eval_summary.json`、`eval_episodes.csv` 和总表 `eval_comparison.csv`。
...
- 500 局 best checkpoint 复评比 10 局 final network 评估更稳，是现在答辩时更推荐引用的学习策略对比表。
- 普通 DQN 的 final network 10 局只有 0.10，但 `best.pt` 500 局有 0.288，说明训练中途确实出现过更好的 DQN 策略。
- 改进 DQN 的 `best.pt` 500 局达到 0.380，高于普通 DQN 的 0.288；在 best checkpoint 口径下，可以说课程学习 + 团队奖励混合带来了正向提升。
- MAPPO 的 `best.pt` 500 局达到 0.556，仍然是三种学习策略中最强；但它低于 final network 10 局的 0.70，说明小样本评估不能过度解读。

面试时推荐结论：

> 我最终更推荐看 best checkpoint 的 500 局复评：普通 DQN 捕获率 28.8%，改进 DQN 38.0%，MAPPO 55.6%。这说明改进 DQN 相对普通 DQN 有提升，而 MAPPO 仍然最强。final network 的 10 局评估我会作为补充，因为它方差更大，不能单独代表算法水平。
...
批量复评一次训练目录下的所有 best checkpoint：

```powershell
.\.venv\Scripts\python -m pursuit_lab.evaluate_runs --runs runs\post_seedfix_full_20260606_164815 --output runs\post_seedfix_best_eval_500 --episodes 500
```
...
src/pursuit_lab/
  ...
  evaluate.py         命令行评估入口
  evaluate_runs.py    批量复评训练目录下的 best.pt checkpoint
  plot.py             训练曲线和评估对比图
  render.py           GIF 生成入口
...
tests/
  ...
  test_evaluate_runs.py        best checkpoint 批量复评测试
...
> post-seedfix 重跑里，DQN 最终网络 10 局评估确实不如随机基线稳定，但训练中途 eval 最高到过 60%。我后来补了 `best.pt` 的 500 局复评，普通 DQN 达到 28.8%，说明 final network 的 10 局评估低估了 DQN 中途较好的 checkpoint。不过它仍然没有 MAPPO 稳定，所以我会把 DQN 作为轻量基线，而不是包装成最强方法。

### Q13：为什么改进版更好？

从 final network 的 10 局评估看，普通 DQN 和改进 DQN 都是 10%，不能直接说改进版最终一定更好。但从 best checkpoint 的 500 局复评看，改进 DQN 捕获率是 38.0%，普通 DQN 是 28.8%，所以在更稳定的 best checkpoint 口径下，改进版确实更好。

但要谨慎：

> 我不会把 DQN 改进版包装成全面成功。它在 best checkpoint 500 局复评中比普通 DQN 更好，说明课程学习和团队奖励混合有正向作用；但它仍然弱于 MAPPO，而且还缺多 seed 和消融实验，所以结论要保守。

### Q13.1：为什么 MAPPO 明显更好？

post-seedfix 完整重跑中，MAPPO final network 10 局捕获率是 70%；best checkpoint 500 局复评中，MAPPO 是 55.6%，仍然高于改进 DQN 的 38.0% 和普通 DQN 的 28.8%。原因是 MAPPO 训练时 critic 能看到全局 observation，能更好地评估团队追捕局面；DQN 每个追捕者主要从自己的 transition 学习，协作信息弱一些。

可以这样回答：

> MAPPO 的优势来自 centralized training decentralized execution。执行时每个追捕者仍然只看本地 observation，但训练时 critic 可以看全局状态，这对多智能体协作追捕很关键。因此在 500 局 best checkpoint 复评中，MAPPO 捕获率 55.6%，高于改进 DQN 的 38.0% 和普通 DQN 的 28.8%。
...
- 当前 trainable 配置最终评估只有 10 局，方差较大；虽然已经补了 `best.pt` 500 局复评，但 final checkpoint 也应统一做 500 局复评。
- 当前还没有多 seed 训练均值和方差，统计说服力不足。
...
2. 统一评估协议，让随机基线、final checkpoint、best checkpoint 都用同样的 500 局 seeds。
...
>
> DQN 改进版加入了课程学习和团队奖励混合。课程学习把 3000 个 episode 分成三个难度阶段，让智能体先学简单任务；团队奖励混合是在每个追捕者自己的 reward 上加入一部分团队平均 reward，引导协作。post-seedfix 重跑里，课程切换在第 1000 和 2000 局附近按预期发生；在 best checkpoint 的 500 局复评中，改进 DQN 捕获率 38.0%，高于普通 DQN 的 28.8%。
>
> 后来我加入 MAPPO，采用 centralized training decentralized execution。actor 执行时只看每个追捕者自己的 16 维 observation，critic 训练时看 62 维全局 observation。MAPPO 训练 3000 episode 后，best checkpoint 的 500 局捕获率达到 55.6%，高于改进 DQN 和普通 DQN。这说明在这个协作追捕任务里，集中式 critic 比 independent DQN 更适合学习团队配合。
